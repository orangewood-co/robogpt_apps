...
class auto_train_implementation(BaseTool):
    ...
    def _run(self,object_to_look: str, object_name: str, forget_old: bool = True, images_to_train: int = 10,number_of_epochs: int = 3 ) -> str:
        ...
        new_weights_path = ExternalServices().send_auto_train_goal(prev_data_folder=previous_data_set, object_name=object_to_look, object_label=object_name, new_weights=forget_old, image_threshold=images_to_train, epochs=number_of_epochs)
        scan_object_implementation()._run(pose_list=None)
...
class scan_object_implementation(BaseTool):
    ...
    def _run(self,pose_list: List[str] = None,init: bool = True):
        home = get_zone_pose_implementation()._run(zone_name="home")
        home1 = get_zone_pose_implementation()._run(zone_name="home1")
        prepick = get_zone_pose_implementation()._run(zone_name="prepick")
        det = get_zone_pose_implementation()._run(zone_name="det")
        
        move_to_pose_implementation()._run(goal_pose=home)
        move_to_pose_implementation()._run(goal_pose=home1)
        move_to_pose_implementation()._run(goal_pose=prepick)
        move_to_pose_implementation()._run(goal_pose=det)

        send_message_to_webapp_implementation()._run(f"Scanning the area using {pose_list} poses")
        if pose_list == None:
            pose_list = ["pos1","pos2","pos3","det"]
        try:
            image_flag = rospy.get_param("/image_capture_end",default=False)
